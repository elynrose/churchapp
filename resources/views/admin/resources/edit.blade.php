...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.resources.update", [$resource->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="link_to">{{ trans('cruds.resource.fields.link_to') }}</label>
                <input class="form-control {{ $errors->has('link_to') ? 'is-invalid' : '' }}" type="text" name="link_to" id="link_to" value="{{ old('link_to', $resource->link_to) }}" required>
                @if($errors->has('link_to'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link_to') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.resource.fields.link_to_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="photo">{{ trans('cruds.resource.fields.photo') }}</label>
