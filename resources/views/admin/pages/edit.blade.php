        ...
        <form method="POST" action="{{ route("admin.pages.update", [$page->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('apps') ? 'is-invalid' : '' }}" name="apps[]" id="apps" multiple required>
                    @foreach($apps as $id => $app)
                        <option value="{{ $id }}" {{ (in_array($id, old('apps', [])) || $page->apps->contains($id)) ? 'selected' : '' }}>{{ $app }}</option>
                    @endforeach
                </select>
                @if($errors->has('apps'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('published') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="published" id="published" value="1" {{ $page->published || old('published', 0) === 1 ? 'checked' : '' }} required>
