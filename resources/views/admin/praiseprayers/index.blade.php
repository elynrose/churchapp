...
@can('praiseprayer_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.praiseprayers.create') }}">
                        ...
                        <tr data-entry-id="{{ $praiseprayer->id }}">
                            ...
                            <td>
                                @can('praiseprayer_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.praiseprayers.show', $praiseprayer->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('praiseprayer_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.praiseprayers.edit', $praiseprayer->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('praiseprayer_delete')
                                    <form action="{{ route('admin.praiseprayers.destroy', $praiseprayer->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('praiseprayer_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.praiseprayers.massDestroy') }}",
