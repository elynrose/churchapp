...
@can('qotd_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.qotds.create') }}">
                ...
                <tbody>
                    @foreach($qotds as $key => $qotd)
                        <tr data-entry-id="{{ $qotd->id }}">
                            ...
                            <td>
                                @can('qotd_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.qotds.show', $qotd->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('qotd_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.qotds.edit', $qotd->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('qotd_delete')
                                    <form action="{{ route('admin.qotds.destroy', $qotd->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('qotd_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.qotds.massDestroy') }}",
