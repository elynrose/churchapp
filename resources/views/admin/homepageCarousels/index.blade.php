...
@can('homepage_carousel_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.homepage-carousels.create') }}">
                        ...
                        <tr data-entry-id="{{ $homepageCarousel->id }}">
                            ...
                            <td>
                                @can('homepage_carousel_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.homepage-carousels.show', $homepageCarousel->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('homepage_carousel_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.homepage-carousels.edit', $homepageCarousel->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('homepage_carousel_delete')
                                    <form action="{{ route('admin.homepage-carousels.destroy', $homepageCarousel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('homepage_carousel_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.homepage-carousels.massDestroy') }}",
