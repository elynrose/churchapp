    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.qotds.update", [$qotd->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="title">{{ trans('cruds.qotd.fields.title') }}</label>
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $qotd->title) }}" required>
                            @if($errors->has('title'))
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="quote" id="quote">{{ old('quote', $qotd->quote) }}</textarea>
                            @if($errors->has('quote'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('quote') }}
                                </div>
...
<script>
    ...
    url: '{{ route('frontend.qotds.storeMedia') }}',
    maxFilesize: 2, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
