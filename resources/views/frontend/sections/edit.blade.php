                    ...
                    <form method="POST" action="{{ route("frontend.sections.update", [$section->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="active" id="active" value="1" {{ $section->active || old('active', 0) === 1 ? 'checked' : '' }}>
                                <label for="active">{{ trans('cruds.section.fields.active') }}</label>
                            </div>
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.section.fields.active_helper') }}</span>
                        </div>
                        <div class="form-group">
                            <label for="description">{{ trans('cruds.section.fields.description') }}</label>
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $section->description) !!}</textarea>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('frontend.sections.storeCKEditorImages') }}', true);
