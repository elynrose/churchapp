        ...
        <div class="col-md-12">
            @can('website_setting_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.website-settings.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $websiteSetting->id }}">
                                        ...
                                        <td>
                                            @can('website_setting_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.website-settings.show', $websiteSetting->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('website_setting_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.website-settings.edit', $websiteSetting->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('website_setting_delete')
                                                <form action="{{ route('frontend.website-settings.destroy', $websiteSetting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('website_setting_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.website-settings.massDestroy') }}",
