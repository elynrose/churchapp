        ...
        <div class="col-md-12">
            @can('uploader_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.uploaders.create') }}">
                            ...
                            <tbody>
                                @foreach($uploaders as $key => $uploader)
                                    <tr data-entry-id="{{ $uploader->id }}">
                                        ...
                                        <td>
                                            @can('uploader_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.uploaders.show', $uploader->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('uploader_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.uploaders.edit', $uploader->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('uploader_delete')
                                                <form action="{{ route('frontend.uploaders.destroy', $uploader->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('uploader_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.uploaders.massDestroy') }}",
