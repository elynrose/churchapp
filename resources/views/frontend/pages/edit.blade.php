            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.pages.update", [$page->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label class="required" for="cover_image">{{ trans('cruds.page.fields.cover_image') }}</label>
                        ...
                        <div class="form-group">
                            <label for="thumb_image">{{ trans('cruds.page.fields.thumb_image') }}</label>
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.page.fields.thumb_image_helper') }}</span>
                        </div>
                        <div class="form-group">
                            <div>
                                <input type="hidden" name="active" value="0">
                                <input type="checkbox" name="active" id="active" value="1" {{ $page->active || old('active', 0) === 1 ? 'checked' : '' }}>
                                <label for="active">{{ trans('cruds.page.fields.active') }}</label>
                            </div>
...
Dropzone.options.thumbImageDropzone = {
    ...
    success: function (file, response) {
      ...
      uploadedThumbImageMap[file.name] = response.name
    },
    removedfile: function (file) {
      console.log(file)
      file.previewElement.remove()
      ...
      if (typeof file.file_name !== 'undefined') {
        ...
        name = uploadedThumbImageMap[file.name]
      }
      $('form').find('input[name="thumb_image[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($page) && $page->thumb_image)
      var files = {!! json_encode($page->thumb_image) !!}
          for (var i in files) {
          var file = files[i]
          this.options.addedfile.call(this, file)
          this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
