        ...
        <div class="col-md-12">
            @can('sister_church_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.sister-churches.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $sisterChurch->id }}">
                                        ...
                                        <td>
                                            @can('sister_church_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.sister-churches.show', $sisterChurch->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('sister_church_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.sister-churches.edit', $sisterChurch->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('sister_church_delete')
                                                <form action="{{ route('frontend.sister-churches.destroy', $sisterChurch->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sister_church_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.sister-churches.massDestroy') }}",
